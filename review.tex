...
% convenience
\nec{\todo}[1]{{\color{red} \textbf{todo: #1}}}
...
\itmz{
\item conditional gaussians
\item bayesian lr and lr
...
\eq{
  ...
  &= \uvnml{y}{x}{\sigma} \uvnml{x}{\mu}{\sigma_0} \uvnml{\sigma}{\mu_\sigma}{\sigma_\sigma} \\
...
\eq{
  p(x,\lambda|\y) &\propto p(\y|x,\lambda) p(x,\lambda) \\
  &= \paren{\frac{\lambda}{2 \pi}}^{\frac{n}{2}} \nexp{\lambda \sum_i (y_i-x)^2}
  \uvnmlp{x}{\mu}{(k\lambda)} \gammadist{\lambda}{\alpha}{\beta} \\
...
\eq{
  ...
  + \lambda \paren{S_{y^2}+k\mu^2} - (n+k)\lambda \paren{\frac{k \mu + S_y}{n+k}}^2\\
...
\eq{
  ...
  \sum_i \qf{y_i}{\Lambda} - 2k\qf{\mu}{\Lambda}\\
}
...
\eq{
(x_a-\mu_a)^T \Lambda_{aa} (x_a-\mu_a) + (x_a-\mu_a)^T \Lambda_{ab} (x_b-\mu_b) + (x_b-\mu_b)^T \Lambda_{ba} (x_a-\mu_a) + (x_b-\mu_b)^T \Lambda_{bb} (x_b-\mu_b)
}
...
\eq{
(x_a-\mu_a)^T \Lambda_{aa} (x_a-\mu_a) - 2 (x_a-\mu_a)^T \Lambda_{ab} \mu_b - \left( \Lambda_{bb}\mu_b - \Lambda_{ba} (x_a-\mu_a) \right)^T \Lambda_{bb}^{-1} \left( \Lambda_{bb}\mu_b - \Lambda_{ba} (x_a-\mu_a) \right)
}


To make our job easier, I'm going to play around one of the terms
\eq{
(x_a-\mu_a)^T \Lambda_{aa} (x_a-\mu_a) - 2 (x_a-\mu_a)^T \Lambda_{ab} \mu_b - \left( \Lambda_{bb}\mu_b - (x_a-\mu_a)^T \Lambda_{ba}^T \right)^T \Lambda_{bb}^{-1} \left( \Lambda_{bb}\mu_b - \Lambda_{ba} (x_a-\mu_a) \right)
}
...
\eq{
  p(x|y) &\propto \nmz p(y|x)p(x) \\
  &= \mvnmlp{y}{Ax+b}{L} \mvnmlp{x}{\mu}{\Lambda} \\
}
...
Now consider $p(y)$, which requires an integral over the conditional distribution.

\todo{everything here is a huge mess}


\eq{
  p(y) &= \int p(y,x) dx \\
  &= \int p(y|x)p(x) dx\\
%  &= \int \mvnmlp{y}{(Ax+b)}{L} dx
}

Notice that this is pretty similar to the computation for $p(x|y)$. Back there, we computed $p(x|y) = \nmz p(x,y)$. We pulled out all the $x$ terms in the joint distribution to get us a normal distribution in $x$. This time, we will pull out all the $x$ terms and recognize that it composes a normal distribution which integrates to 1, or some constant. Then, we will use the left over $y$ terms in order to figure out the distribution.

The terms inside the exponential of the joint distribution are:

\eq{
  \qfo{y}{L}{(Ax+b)} + \qfo{x}{\Lambda}{\mu} \\
}

Recall that the quadratic terms in $x$ are $A^T L A + \Lambda$ and the linear terms are $-2\paren{A^TL(y-b) + \Lambda \mu}$. If we complete the square, we end up with a quadratic term which contains all the $x$, and an additinal left over term:

\eq{
  \qf{\paren{A^TL(y-b)+\Lambda \mu}}{\paren{A^T L A + \Lambda}}
}

In addition to this term, we can pull out all the $y$ terms from the first part of the joint distribution which we did not use to complete the square:

\eq{
  \qf{y}{L} -2y^TLb
}

Now all that remains is to figure out the quadratic and linear terms in $y$.

The quadratic terms:
\eq{
  L + LAA^TLAA^TL + LA^T \Lambda A^T L
}

We are trying to arrive at the correct soltion:

\eq{
  \paren{\minv L + A \minv \Lambda A^T}^{-1}
}

We expect linear terms:
...
So we end up with the distribution for $y$:
...
We are computing the entropy of the multivariate gaussian.
\eq{
  ...
  \right) dx\\
  &=  -\log \left( (2\pi)^{-\frac{D}{2}}  | \Sigma |^{-\frac{1}{2}} \right) -\int (2\pi)^{-\frac{D}{2}}  | \Sigma |^{-\frac{1}{2}} \exp \left( -\frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu) \right)
  \log \left( 
    \exp \left( -\frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu) \right)
  \right) dx\\
  &=  \frac{D}{2} \log(2\pi) + \frac{1}{2} \log |\Sigma| - \int (2\pi)^{-\frac{D}{2}}  | \Sigma |^{-\frac{1}{2}} \exp \left( -\frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu) \right)
  \left( -\frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu) \right) dx\\
...
\eq{
  ...
  \right]\\
  &= E_p \left[ \frac{1}{2} \log |L| - \frac{1}{2} \log |\Sigma| + 
    \log \left(
      \exp \left(
...
\eq{
    ...
    &= \sum_{a} \gradt \pol(a|s) q_{\pol}(s, a) 
    + \gamma \pol(a|s) \paren{\sum_{s'} P(s'|a, s) \paren{\sum_{a} \gradt \pol(a|s) q_{\pol}(s, a) + \gamma \pol(a'|s') \paren{\cdots}}}\\
}
...
\eq{
  ...
  &= 4x^Tx x^T (dx) - 4 x^T Z dx & \text{assuming $Z$ is symmetric}\\
}
