...
\itmz{
  \item PCA 
  \item RL or at least COMA
  \item various NN architectures
  \item optimization    
...
Next is the question of finding the eigenvectors and values of a matrix. 

\todo{add in some SVD and ED}
...
\eq{
  \grad p(x) &\approx \avgsum_i \grad_\theta p_\theta(x|z_i)
}
...
\eq{
  \grad p(x) &\approx \avgsum_i \grad_\theta \paren{p_\theta(x|z_i) \frac{p(z_i)}{q(z_i)}}
}

Indeed, it makes sense for $q$ to be different depending on $x$. 

\eq{
  \grad p(x) &\approx \avgsum_i \grad_\theta \paren{p_\theta(x|z_i) \frac{p(z_i)}{q(z_i|x)}}
}
...
Again, we parameterize the probability distribution with a neural network. Now the question remains: How can we train the encoder $q$ to approximate the true $p(z|x)$ in order to minimize variance?
