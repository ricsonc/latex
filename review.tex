...
\eq{
  p(\Lambda) &\sim \mathcal{W}({\alpha}{\beta}) \\
  p(x) &\sim \nml{\mu}{k L}
}

The Wishart probability distribution $\mathcal{W}$ has density:

\eq{
  p(\Lambda) &= \frac{1}{Z} |\Lambda|^\alpha \bexp{-\beta |\Lambda|}
}

Given both of these, we can now compute the posterior:
...
\eq{
  ...
  &= \qf{x}{A} + \qf{(\minv A b)}{A} - x^T A (\minv A b) - \qf{b}{A} \\
  &= \qf{x}{A} + b^T A^{-1^T} A\minv A b - x^Tb - \qf{b}{A} \\
  &= \qf{x}{A} + b^T A^{-1^T} b - x^T b - \qf{b}{\minv A} \\
  &= \qf{x}{A} - b^T x \\
}
