...
\itmz{
  \item finish section on conjugate priors
  \item write section on RL (or at least COMA)
  \item write section on various NN architectures
  \item optimization    
...
\eq{
  ...
  %\log P(\y|x,\lambda) &= \frac{n}{2} \log \frac{\lambda}{2 \pi} - \lambda \sum_i (y_i-x)^2
}
...
Now we can complete the terms with respect to $x$. Given that $A = (n+k)\Lambda$ and $b = 2k\Lambda \mu$, this comes out to

\eq{
  &\qfo{x}{A}{\minv A b} - \qf{b}{A} + \sum_i \qf{y_i}{\Lambda} - 2k\qf{\mu}{\Lambda} \\
  &= \qfo{x}{(n+k)\Lambda}{\minv{((n+k)\Lambda)}b} - \qf{(2k\Lambda \mu)}{((n+k)\Lambda)} +
  \sum_i \qf{y_i}{\Lambda} - 2k\qf{\mu}{\Lambda}\\
}

So the extra terms are
\eq{
  & \sum_i \qf{y_i}{\Lambda} - 2k\qf{\mu}{\Lambda} - \qf{(2k\Lambda \mu)}{((n+k)\Lambda)}\\
  &= \sum_i \qf{y_i}{\Lambda} - 2k\qf{\mu}{\Lambda} - 4k^2(n+k) \qf{(\Lambda \mu)}{\Lambda} \\
}
...
\eq{
  \Omega &= \sum_i \scov{y_i} + 2k \scov{\mu} - 4k^2(n+k) \scov{ (\Lambda \mu) } \\
  &= \sum_i \scov{y_i} + 2k \scov{\mu} - 4k^2(n+k) \Lambda {\scov{\mu}} \Lambda \\
}

Then the extra terms come out to $\tr(\Omega \Lambda)$. Now we know that our prior $p(\Lambda)$ must have something to the effect of $\bexp{\nhaf tr(M\Lambda)}$ in it. Furthermore, $M$ must be a function of $V$, which is the only matrix parameter in the wishart distribution. We choose $M = \minv V$.
