...
So the derivative comes out to $\tr(X^T \Sigma X)$. We want to find the inverse of this matrix and take the eigenvectors, because the sum of those yields the trace.

\eq{
  (X^T \Sigma X)^{-1} &= \minv X (X^T \Sigma)^{-1} \\
  &= \minv X \minv \Sigma (X^T)^{-1} \\
  &= \minv X (\alpha I + \beta XX^T) (X^T)^{-1} \\
  &= \alpha \minv X (X^T)^{-1} + \beta \minv X X X^T (X^T)^{-1} \\
  &= \alpha (X^TX)^{-1} + \beta I\\
}
...
\eq{
  \alpha (X^TX)^{-1} v_i + \beta I v_i &= \lambda_i v_i \\
  \alpha (X^TX)^{-1} v_i &= (\lambda_i - \beta) v_i
}

And the gradient of the log determinant comes out to:

\eq{
  \sum_i \frac{1}{\lambda_i + \beta}
}
