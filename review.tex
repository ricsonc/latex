...
The goal of these notes is to provide fully worked derivations for common topics without skipping any steps.
...
We can simply examine the guts of this expression. At this point, notice that we only need the quadratic and linear terms of $x$. 
...
\eq{
  \kl{p}{q_\theta} &= -E_p \sparen{ \log \frac{p(x)}{q(x)}} \\
  &= -E_p[\log p(x) - \log q(x)] \\
...
Suppose we are trying to predict labels $t$ from data $x$. Let our prediction function be $y(x) : x \rightarrow t$. In addition, we have a loss function $L(t, y)$. We wish to minimize the expected loss:

\eq{
  E_{x,t} [L(t,y)] &= \int \int L(t,y(x)) p(x,t) dx dt \\
}

\subsubsection{Continuous Domain}

One special case of the loss, Minkowsky loss, is of the form $L(t, y) = |y-t|^q$. 

\eq{
  E[L] &= \int \int |y(x) - t|^q p(x,t) dt dx \\
  &= \int \int |y(x) - t|^q p(t|x)\, dt\, p(x)\,dx \\
}

Now consider any value of $x$. We want to pick the value $y$ which minimizes the following integral. Consider $q = 1$.
\eq{
  \int |y - t| p(t|x)\, dt
}

We want to set the derivative of this expression with respect to $y$ to be 0, in order to minimize the expression:
\eq{
  \frac{\partial}{\partial y} \int |y-t| p(t|x)\, dt &= \int \frac{\partial}{\partial y} |y-t| p(t|x)\, dt \\
  &= \int \text{sgn}(y > t)p(t|x) dt
  \intertext{Where sgn is the sign function, so that it evaluates to 1 when $y > t$ and -1 when $y < t$ and 0 otherwise.}
  &= -\int_{-\infty}^y p(t|x) dt + \int_y^\infty p(t|x) dt
}


If we set this to zero, we get the equation:
...
Therefore, the optimal $y(x)$ is the median of the conditional distribution $t|x$. 

Now let's try $q = 0$. The expected loss is:

\eq{
  \int \int \mathbf{1}[y(x) = t] p_t(t|x)\, dt\, p(x)\, dx
}

Where $\mathbf{1}$ evaluates to 1 if the expression inside is true and otherwise evaluates to 0. Just like the previous problem, we pick an arbitrary fixed $x$ and look at the inner integral:

\eq{
  &\int \mathbf{1}[y = t] p_t(t|x)\, dt
  \intertext{The integrand is 0 unless $y = t$, leaving us with the following quantity}
  &= p_t(y|x)\, dt
}

Now we know the optimal function $y(x)$ will maximize $p_t(y|x)$. In other words, it is the mode of the conditional distribution $t|x$.

\subsubsection{Discrete Domain}

For discrete problems, we can consider $L(t,y)$ as a matrix of loss values.

\eq{
  E[L] &= \sum_t \int L_{t,y(x)} P(x,t) dx \\
  &= \sum_t P(t) \int L_{t,y(x)} P(x|t) dx \\
  &= \sum_t P(t) \int L_{t,y(x)} P(x,t) dx \\
  &= \sum_t P(t) \sum_y \int_{x \in R_y} L_{t, y} P(x|t) dx
  \intertext{where $R_y$ denotes the set of $x$ such that $y(x) = y$}
  &= \sum_t P(t) \sum_y L_{t,y} \int_{x \in R_y} P(x|t) dx \\
  &= \sum_t P(t) \sum_y L_{t,y} \int_{x \in R_y} P(t|x)\frac{P(x)}{P(t)} dx \\
  &= \sum_t \sum_y L_{t,y} \int_{x \in R_y} P(t|x)P(x) dx \\
  &= \sum_y \int_{x \in R_y} P(x) \sum_t L_{t,y} P(t|x) dx \\
}

Therefore, we want to put each $x$ in the region $R_y$ which minimizes the integrand:

\eq{
  c(y, x) &= P(x) \sum_t L_{t,y} P(t|x) \\
  &\propto \sum_t L_{t,y} P(t|x) \\
  R_{y^*} &= \{x | y^* = \argmin_{y} c(y,x) \} \\
  y(x) &= y: x \in R_y
}
...
Often, we will deal with models with both observed and unobserved variables. For example, in part of speech tagging, the words are observed variables where as the POS tag of each word is unobserved. In a clustering problem, the position of each point is observed by the label on each point is unobserved. Both the EM algorithm and variational inference are used to deal with these types of models.

\subsection{Expectation Maximization }

There are three important variables: the observed variables $X$, the hidden variables $Z$ and the parameters of the model $\theta$. We want to solve the following optimization problem:

\eq{
  \max_\theta P(X|\theta)
}

But we only have a tractable form for the joint distribution $P(X,Z|\theta)$. Note that naive marginalization is intractable:

\eq{
  P(X|\theta) &= \sum_Z P(X,Z|\theta)
}

However, we can also break this into an expectation:
\eq{
  P(X|\theta) &= \sum_Z P(Z)P(X|Z, \theta) \\
  &= E_{Z \sim P} [P(X|Z, \theta)]
}

Unfortunately, we do not have the distribution $P(Z)$ either (and it is intractable to compute. But this gives us the idea to consider the expectation under some other arbitrary distribution $q$.

\eq{
  \log P(X|\theta) &= \log P(X|\theta) \sum_Z q(Z) \\
  &= \sum_z q(Z) \log P(X|\theta) \\
  &= E_{Z \sim q} [\log P(X|\theta)] \\
  &= E_{Z \sim q} \sparen{ \log \frac{P(X|\theta)P(Z|X, \theta)}{P(Z|X, \theta)} } \\
  &= E_{Z \sim q} \sparen{ \log \frac{P(X,Z|\theta)}{P(Z|X,\theta)} } \\
  &= E_{Z \sim q} \sparen{ \log P(X,Z|\theta) - \log q(Z) + \log q(Z) - P(Z|X,\theta) } \\
  &= E_{Z \sim q} \sparen{ \log \frac{P(X,Z|\theta)}{q(Z)} + \log \frac{q(Z)}{P(Z|X,\theta)} } \\
  &= E_{Z \sim q} \sparen{ \log \frac{P(X,Z|\theta)}{q(Z)}} + E_{Z \sim q} \sparen{ \log \frac{q(Z)}{P(Z|X,\theta)} } \\
  &= \lag(q, \theta) + \kl{q}{P}
}

Where $\lag$ is called the variational lower bound. 

Now the algorithm is simply repeating the following two steps: expectation -- which is maximizing $\lag$ with respect to $q$, and maximization, which is maximizing $\lag$ with respect to $\theta$. This will converge to the optimal $\theta$, which is what we want. Note that we don't directly optimize $P(X|\theta)$ but instead maximizing a lower bound on the quantity.

How can we maximize $\lag$ with respect to $q$? We can try minimizing the second term $\kl{q}{P(Z|X,\theta)}$ instead, this is trivially done by setting $q(Z) = P(Z|X,\theta)$. This means that when $\theta$ converges to some maxima $\theta^*$, then the kl divergence will be zero, so that the lower bound is exactly the log likelihood.

To summarize, here is the algorithm for EM:
\enum{
\item Fix $q = P(Z|X, \theta)$
\item Find an analytical form for $\lag(q, \theta)$
\item Maximize $\lag$ with respect to $\theta$
\item repeat until converged
}

Note that step 2 is computing the expectation, which is why it is called the expectation step. Step 3 is maximization, giving us the name Expectation-Maximization.

\subsubsection{Gaussian Mixture Model}

A gaussian mixture model is one such example of a hidden variable model. There are $k$ gaussian components composed together in a mixture density, and there are $n$ datapoints. Each datapoint was generated from the mixture. The task is to infer the mixture means and covariances. In other words, $X$ are the datapoint positions, $Z$ are which mixture component each datapoint ``belongs'' to and $\theta = (\mu, \Sigma, \pi)$ are the means, covariances, and weights of each mixture component. The joint distribution for this model can be written:

\eq{
  &P(X,Z|\mu, \Sigma, \pi) \\
  &= \prod_i \prod_j P(X_i|\mu_j,\Sigma_j, \pi_j)^{Z_{ij}} \\
  &= \prod_i \prod_j \paren{\pi_j \mvnml{X_i}{\mu_j}{\Sigma_j}}^{Z_{ij}} \\
  &\log P(X,Z|\mu,\Sigma, \pi) \\
  &= \sum_i \sum_j Z_{ij} \log \paren{\pi_j \mvnml{X_i}{\mu_j}{\Sigma_j}} \\
  &= \sum_i \sum_j Z_{ij} \paren{\log \pi_j \nhaf \log |\Sigma_j| \nhaf \qfo{X_i}{\minv \Sigma_j}{\mu_j}} + \const \\
  &E_{Z \sim q}[\log P(X,Z|\mu, \Sigma, \pi)] \\
  &= \sum_i \sum_j E_q[Z_{ij}] \paren{\log \pi_j \nhaf \log |\Sigma_j| \nhaf \qfo{X_i}{\minv \Sigma_j}{\mu_j}} + \const \\
}

Define
\eq{
  \gamma_{ij} &= E_q[Z_{ij}]
  \intertext{Now let $q = P(Z|X,\theta)$} 
  &= P(Z_{ij}=1|X, \theta) \\
  &= \frac{P(X|Z_{ij}=1, \theta)P(Z_{ij}=1|\theta)}{P(X)} \\
  &= \frac{P(X|Z_{ij}=1, \theta)P(Z_{ij}=1|\theta)}{\sum_{j'} P(Z_{ij'}=1|X,\theta)} \\
  &= \frac{P(X|Z_{ij}=1, \theta)P(Z_{ij}=1|\theta)}{\sum_{j'} P(X|Z_{ij'}=1, \theta) P(Z_{ij'}=1|\theta)} \\
  &= \frac{P(X|Z_{ij}=1, \theta) \pi_j }{\sum_{j'} P(X|Z_{ij'}=1, \theta) \pi_{j'}} \\
}

Furthermore, recall that by definition of a gaussian density

\eq{
  P(X|Z_{ij}=1) &= \mvnml{X_i}{\mu_j}{\Sigma_j} 
}

So now we are able to compute $\gamma_{ij}$, so we have a closed form for $E_{Z \sim q}[\log P(X,Z|\mu, \Sigma, \pi)]$. Moreover, $\lag(q,\theta) = E_{Z \sim q}[\log P(X,Z|\mu, \Sigma, \pi)] + H[q]$ and the entropy term is constant with respect to $\theta$. Therefore the next step is to maximize $E_{Z \sim q}[\log P(X,Z|\mu, \Sigma, \pi)]$ with respect to $\mu$, $\Sigma$, and $\pi$. Note that when we maximize, we treat $\gamma_{ij}$ as a fixed constant, even though the computation of $\gamma$ depends on $\theta$. We do this because $\gamma$ was actually derived from an expectation of some variational distribution $q$, and $q$ is supposed to be fixed.

Now we have the following constrained optimization problem:

\eq{
  & \max_{\mu, \Sigma, \pi} \sum_i \sum_j \gamma_{ij} \paren{\log \pi_j \nhaf \log |\Sigma_j| \nhaf \qfo{X_i}{\minv \Sigma_j}{\mu_j}}\\
  \st & \sum_j \pi_j = 1
}
...
\eq{
  \lag(\mu, \Sigma, \pi, \lambda) &= \sum_i \sum_j \gamma_{ij} \paren{\log \pi_j \nhaf \log |\Sigma_j| \nhaf \qfo{X_i}{\minv \Sigma_j}{\mu_j}} \\
  & \qquad + \lambda \paren{ \sum_j \pi_j -1 }
}

Now we will solve for $\lambda$ first:

\eq{
  \grad_{\pi_k} \lag &= \frac{1}{\pi_k} \sum_i \gamma_{ik} + \lambda \\
  \intertext{For convenience, denote $\Gamma_k = \sum_i \gamma_{ik}$}
  0 &= \frac{\Gamma_k}{\pi_k}+\lambda \\
  \pi_k &= -\frac{\Gamma_k}{\lambda} \\
  \sum_j \pi_j &= - \sum_j \frac{\Gamma_k}{\lambda} \\
  1 &= -\sum_j \frac{\Gamma_k} {\lambda} \\
  \lambda &= -\sum_j \Gamma_k \\
  \pi_k &= \frac{\Gamma_k}{\sum_j \Gamma_j}
}

This gives us the updated values of $\pi$. Intuitively since, $\gamma_{ij}$ is the probability that point $i$ is in mixture component $j$, $\Gamma_j$ is the expected number of points in mixture component $j$. So the weight on each cluster is precisely the expected number of points in that mixture over the total expected number of points in all mixtures. 

Now for the update of $\mu$:
\eq{
  \grad_{\mu_k} \lag &= \sum_i \gamma_{ik} \grad_{\mu_k}
  \paren{X_i^T \minv \Sigma_k \mu_k \nhaf \qf{\mu_k}{\minv \Sigma_k} } \\
  &= \sum_i \gamma_{ik} \paren{ (\minv \Sigma_k)^T X_i - \minv \Sigma_k \mu_k } \\
  &= \minv \Sigma_k \sum_i \gamma_{ik} (X_i-\mu_k)
}

Assuming $\minv \Sigma_k$ isn't degenerate in some way, this means:

\eq{
  0 &= \sum_i \gamma_{ik} (X_i-\mu_k) \\
  \mu_k \Gamma_k &= \sum_i \gamma_{ik} X_i \\
  \mu_k &= \sum_i \frac{\gamma_{ik}}{\Gamma_k} X_i 
}

This should intuitively make sense -- each component mean is the weighted average of all points in the dataset, where the average is weighted by how much a point belongs to that mixture component.

And finally for $\Sigma$: 
\eq{
  \grad_{\Sigma_k} \lag &= \sum_i \gamma_{ij} \paren{\nhaf} \paren{ \grad_{\Sigma_k} (\log |\Sigma_k|) + \grad_{\Sigma_k} \qfo{X_i}{\minv \Sigma_k}{\mu_k} } 
}

Recall that $\grad \log |\Sigma_k| = \minv \Sigma_k$. Also for the second term, we can take the gradient using traces:

\eq{
  d \qfo{X_i}{\minv \Sigma_k}{\mu_k} &= d \tr \paren{ \qfo{X_i}{\minv \Sigma_k}{\mu_k} } \\
  &= d \tr \paren{ \scov{(X_i-\mu_k)} \minv \Sigma_k } \\
  &= \tr \paren{ \scov{(X_i-\mu_k)} d\minv \Sigma_k } \\
  &= \tr \paren{ \scov{(X_i-\mu_k)} \paren{-\minv \Sigma_k d \Sigma_k \minv \Sigma_k} } \\
  &= -\tr \paren{\minv \Sigma_k  \scov{(X_i-\mu_k)} \minv \Sigma_k d\Sigma_k }\\
  \grad_{\Sigma_k} \qfo{X_i}{\Sigma_k}{\mu_k} &= -\minv \Sigma_k \scov{(X_i-\mu_k)} \minv \Sigma_k
}

Plugging these values back in:

\eq{
  0 &= \sum_i \gamma_{ik} \paren{\nhaf} \paren{ \minv \Sigma_k - \minv \Sigma_k \scov{(X_i-\mu_k)} \minv \Sigma_k} \\
  \minv \Sigma_k \Gamma_k &= \sum_i \gamma_{ij} \minv \Sigma_k \scov{(X_i-\mu_k)} \minv \Sigma_k\\
  \minv \Sigma_k \Gamma_k &= \minv \Sigma_k \paren{\sum_i \gamma_{ij} \scov{(X_i-\mu_k)} }\minv \Sigma_k\\
  \Gamma_k &= \paren{\sum_i \gamma_{ij} \scov{(X_i-\mu_k)} }\minv \Sigma_k\\
  \Gamma_k \Sigma_k &= \sum_i \gamma_{ij} \scov{(X_i-\mu_k)} \\
  \Sigma_k &= \sum_i \frac{\gamma_{ij}}{\Gamma_k} \scov{(X_i-\mu_k)}
}

And again this matches up with our intuition.

In conclusion, the GMM inference algorithm with expectation maximization alternates between recomputing values of $\gamma$ and values for $\mu$, $\Sigma$, and $\pi$.

\subsubsection{Hard EM}

Apparently there is a popular variant of EM called hard EM which replaces the expectation with another maximization step. Another way to look at it is that we restrict $q$ to be a point distribution.

In soft EM, we had the following lower bound:

\eq{
  \lag(q, \theta) &= E_{Z \sim q} \sparen{ \log \frac{P(X,Z|\theta)}{q(Z)}}
}

Here is what hard EM looks like:

\eq{
  \lag(\theta) &= \max_Z \log P(X,Z|\theta)
}

This really just makes hard EM and alternation of the optimization of $Z$ and $\theta$. 

\subsection{Variational Inference}

In approximate inference, the setup is very similar. Recall the decomposition:

\eq{
  \log p(X) &= \lag(q) + \kl{q}{p}
}

In variational inference, we constrain $q$ to be a tractable distribution which is easy to work with. One common choice of $q$ is a factorized distribution. For example, if $Z$ is a set of variables $z_i, \cdots, z_n$ then we can force $q$ to be in the form $q(Z) = \prod_i q_i(z_i)$. Since $q$ is now tractable, hopefully we can maximize $\lag(q)$ now.

Why is this approach necessary when we already have the EM algorithm? I suspect, but am not sure, that this algorithm is more general and works in situations where EM does not. In particular, the maximization step of EM algorithm might not have a closed form in some cases.

Now all that remains is to solve for $q_1, \cdots, q_n$ which maximizes $\lag(q)$. For convenience, we abuse notation and write $q_i(z_i)$ as $q_i$. 

\eq{
  \lag(q) &= \int q(Z) \log \frac{P(X,Z)}{q(Z)} dZ \\
  &= \int \prod_i q_i \paren{ \log P(X,Z) - \sum_i \log q_i } dZ \\
  \intertext{Now let's try to single out a particular $q_k$}
  &= \int \int q_k \prod_{i \neq k} q_i \paren{ \log P(X,Z) - \sum_{i\neq k} \log q_i - \log q_k} dZ_{i\neq k} dZ_k\\
  &= \int \int q_k \prod_{i \neq k} q_i \paren{ \log P(X,Z) - \sum_{i\neq k} \log q_i} dZ_{i\neq k} dZ_k \\
  &\qquad - \int \int q_k \prod_{i \neq k} q_i \log q_k dZ_{i\neq k} dZ_k\\
  &= \int q_k \paren{ \int \prod_{i \neq k} q_i \paren{ \log P(X,Z) - \sum_{i\neq k} \log q_i } dZ_{i\neq k} } dZ_k \\
  &\qquad - \int q_k \log q_k \paren{ \int \prod_{i\neq k} q_i dZ_{i\neq k} } dZ_k\\
  &= \int q_k \paren{ \int \prod_{i \neq k} q_i \paren{ \log P(X,Z) - \sum_{i\neq k} \log q_i } dZ_{i\neq k} } dZ_k \\
  &\qquad - \int q_k \log q_k dZ_k\\
}

Let's simplify the inner integrand:

\eq{
  &\int \prod_{i \neq k} q_i \paren{ \log P(X,Z) - \sum_{i\neq k} \log q_i } dZ_{i\neq k} \\
  &= \int q_{i\neq k}(Z) \log P(X,Z)\, dZ_{i\neq k} - \int q_{i\neq k}(Z) \sum_{i\neq k} q_i\, dZ_{i\neq k} \\
  \intertext{Since we are maximizing with respect to $q_k$, and the second term doesn't contain any variables associated with $q_k$ or $x_k$ or $z_k$, we can treat it as constant.}
    &= E_{q_{i\neq k}}[\log P(X,Z)] + \const
}

Note that this expression is a function of $Z_k$ because $Z_k$ is only specified in the outer integrand:

\eq{
  \lag(q) &= \int q_k \paren{E_{q_{i\neq k}}[\log P(X,Z)] +\const} dZ_k - \int q_k \log q_k\, dZ_k  \\
}

Notice the following identity:

\eq{
  \kl{q}{\tilde p} &= -\int q(z) \paren{\log \tilde p(z) - \log q(z)} dz
}

Now if we want to minimize the kl-divergence, we simply set $q = \tilde p$. This would minimize the integral:
\eq{
  \int q(z) \paren{\log \tilde p(z) - \log q(z)} dz
}

This integral closely parallelizes the integral we want to solve, so it is clear what $\tilde p$ is now.

\eq{
  \log \tilde p(Z_k) &= E_{q_{i\neq k}}[\log P(X,Z)] + \const\\
  q^*(Z_k) &\propto \bexp{ E_{q_{i\neq k}}[\log P(X,Z)] }\\
}

Usually normalization of $q^*$ isn't performed because it isn't useful.

\subsubsection{Dense Gaussian CRFS}

A very popular class of CRFs are dense gaussian crfs, used for image processing. In this case, variables are arranged in an n by n grid (one variable per pixel), with unary potentials for every variable on the grid and pairwise potentials between every pair of variables. For a medium sized image with $10^7$ pixels, this means there are over 100 trillion potentials, making this problem intractable to solve for traditional methods.

The task is to solve for the posterior distribution on $Z$, given X. The distribution on $Z$ is a discrete distribution for each pixel which represents the segmentation class of that pixel.
...
\eq{
\intertext{We will satisfy the conditions detailed balance, which is true of many stationary markov chains (but not strictly required). This should intuitively make sense}
P(x_{t+1} = x',x_t = x) &= P(x_{t+1} = x,x_t = x')
\intertext{We will drop some of the notation for conciseness}
P(x'|x)P(x) &= P(x|x')P(x)
}
...
\eq{
  A(x'|x) \pi(x'|x) P(x) &= A(x|x') \pi(x|x') P(x') \\
  \frac{A(x'|x)}{A(x|x')} &= \frac{P(x') \pi(x|x')}{P(x) \pi(x'|x)} \\
  &= \frac{P(x')}{P(x)}
}
...
\eq{
  A(x'|x) &= \min \paren{\frac{P(x')}{P(x)}, 1}
}
...
\eq{
  \frac{A(x'|x)}{A(x|x')} &= \frac{1}{\frac{P(x)}{P(x')}} \\
  &= \frac{P(x')}{P(x)}
}
...
\eq{
  A(x'|x) &= \min \paren{\frac{P(x') \pi(x|x')}{P(x) \pi(x'|x)}, 1}
}
...
Formally speaking, in normal policy gradients, the task is the following constraint maximization:

\eq{
  \max_{\Delta \theta} J(\theta+ \Delta \theta) \\
  \st ||\Delta \theta|| < \epsilon
}

In natural policy gradients, the problem can be rephrased in terms of the policy $\pi$. We abuse notation and allow $J$ to take in a policy instead of the parameter.
...
Where $\pi$ was the previous policy and $d$ is some measure of distance. What is a suitable distance function? Since $\pi$ is a policy, something like KL-divergence may be a good idea, however, this is not well-defined because $\pi$ is a different policy for every state.

Recall $\mu_\pi(s)$ from the discussion on policy gradients -- which is the expected discounted visitation counts to state $s$ under policy $\pi$. This gives us the idea of letting $d$ be the expected KL-divergence over all states where the expectation is from the distribution $\mu_\pi$. Formally, we say:

\eq{
  d(\pi, \pi') &= E_{\mu_\pi} \sparen{ \kl{\pi}{\pi'} }
}
...
\eq{
  &\max_{\pi'} J(\pi') \\
  \st &E_{\mu_\pi} \sparen{ \kl{\pi(s)}{\pi'(s)} } < \epsilon 
}

This can be converted back to the parameterized form:

\eq{
  &\max_{\theta'} J(\theta') \\
  \st &E_{\mu_{\pi_\theta}} \sparen{ \kl{\pi_\theta(s)}{\pi_{\theta'}(s)} } < \epsilon 
}

We will approximate the KL-divergence of the new distribution with a second order taylor expansion. To be painfully precise and avoid confusion, we define a new function $g_s$ for each state $s$:

\eq{
  g_s(\theta') = \kl{\pi_\theta(s)}{\pi_{\theta'}(s)}
}

And we approximate $g_s$ with a taylor expansion around $\theta$. As it happens, the zeroth and first order terms are zero because $\theta$ is a local minimum of $g_s$ and $g_s(\theta) = 0$. Not only that, but recall that the hessian of KL-divergence is the fisher matrix, so we have:

\eq{
  (H_{\theta'}) g_s &= F_s(\theta')
}

Therefore we have the approximation:

\eq{
  g(\theta') &\approx \haf \qfo{\theta'}{F_s}{\theta} \\
  &= \qf{\dtheta}{F_s} \\
  E_{\mu_{\pi_\theta}} \sparen{ \kl{\pi_\theta(s)}{\pi_{\theta'}(s)} } &= \haf E[ \qf{\dtheta}{F_s} ] \\
  &= \haf \qf{\dtheta}{F}
}

Where we define the expected fisher matrix $F = E_{s \sim \mu}[F_s]$. In practice, each $F_s$ can be computed easily using automatic differentiation.

We must now solve the constrained optimization problem:

\eq{
  &\max_{\dtheta} J(\theta + \dtheta) \\
  \st &\haf \qf{\dtheta}{F} < \epsilon
}

The lagrangian to this constrained optimization problem is:

\eq{
  \lag(\dtheta, \lambda) &= J(\theta + \dtheta) + \lambda \paren{\haf \qf{\dtheta}{F} - \eps} \\
  \grad_{\dtheta} \lag &= \grad_{\dtheta} J(\theta+\dtheta) + \lambda F \dtheta \\
  &= \grad_{\theta'} J(\theta') + \lambda F \dtheta
  \intertext{We want to set this quantity to be 0}
  \grad J &= - \lambda F \dtheta \\
  \dtheta &= -\frac{1}{\lambda} \minv F \grad J 
}

Therefore it is apparent that the direction of steepest descent is $\minv F \grad J$.
...
\eq{
  ...
  &\qquad +\left( \lambda_1 \int f(x) dx -1 \right)\\
  &\qquad +\left( \lambda_2 \int xf(x) dx \right)\\
  &\qquad +\left( \lambda_3 \int x^2 f(x) dx -1 \right) \\
  \lambda &\neq 0\\
  &\nabla_x L(x,\lambda) = 0
}
...
This expression can now be optimized using your favorite convex optimization algorithm. One simple algorithm which works well is dual coordinate ascent. Simply select two $\lambda_i$ and $\lambda_j$ and line search greedily for a better solution. Repeat. When finished, we can recover an expression for $v$.
