...
\eq{
    ...
    x^T D_x^T P_{t+1} D_u u_t + u^T D_u^T P_{t+1} D_x x \\
}
...
\eq{
	J_t(x) &= \qf{x}{C_x} + \qf{(Kx)}{C_u} + \qf{x}{D_x^T P_{t+1} D_x} + \qf{(Kx)}{D_u^T P_{t+1} D_u} + 
    	x^T D_x^T P_{t+1} D_u K x + x^T K^T D_u^T P_{t+1} D_x x \\
    &= \qf{x}{\paren{C_x + K^TC_uK + D_x^T P_{t+1} D_x + K^T D_u^T P_{t+1} D_u K + D_x^T P_{t+1} D_u K + K^T D_u^T P_{t+1} D_x}} \\
    &= \qf{x}{P_t}
}
